...
class RobotControlPanel(QWidget):
    ...
    def initUI(self):
        ...
        self.map_btn.setStyleSheet("""
            QPushButton {
                background-color: white; border: 2px solid #0056b3; color: #333;
                font-size: 16px; font-weight: bold; border-radius: 12px;
            }
            QPushButton:hover { background-color: #E8F0FE; }
            QPushButton:pressed { padding-top: 5px; padding-left: 5px; background-color: #D2E3FC; }
        """)
        self.map_btn.setFixedHeight(85); page_main_layout.addWidget(self.map_btn)
        ...
        # ================= [ 3. 맵 팝업 오버레이 (구성 수정) ] =================
        self.map_overlay = QFrame()
        self.map_overlay.setStyleSheet("background-color: rgba(0, 0, 0, 150);")
        self.map_overlay.hide()

        self.map_popup_box = QFrame(self.map_overlay)
        self.map_popup_box.setFixedSize(850, 700) # 더 넓은 뷰를 위해 사이즈 소폭 확장
        self.map_popup_box.setStyleSheet("background-color: white; border-radius: 30px; border: 2px solid #D1D9E6;")
        m_layout = QVBoxLayout(self.map_popup_box)
        m_layout.setContentsMargins(30, 30, 30, 30)

        m_title = QLabel("로봇 실시간 위치")
        m_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        m_title.setStyleSheet("font-size: 26px; font-weight: 900; color: #2C3E50; border: none;")
        m_layout.addWidget(m_title) # 상단 배치

        self.map_area = QFrame()
        self.map_area.setStyleSheet("background-color: #F8F9FA; border: 1px solid #DDE4ED; border-radius: 15px;")
        map_placeholder = QLabel("MAP GRID AREA")
        map_placeholder.setAlignment(Qt.AlignmentFlag.AlignCenter)
        map_placeholder.setStyleSheet("font-size: 20px; color: #ADB5BD; font-weight: bold;")
        QVBoxLayout(self.map_area).addWidget(map_placeholder)

        # 맵 영역이 중앙에서 최대한 많은 공간을 차지하도록 설정
        m_layout.addWidget(self.map_area, stretch=1)
        m_layout.addSpacing(20)

        self.btn_map_close = QPushButton("닫기")
        self.btn_map_close.setFixedHeight(65)
        self.btn_map_close.setStyleSheet("""
            QPushButton {
                background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #28A745, stop:1 #1E7E34);
                color: white; border-radius: 18px; font-size: 22px; font-weight: bold;
            }
            QPushButton:hover { background-color: #218838; }
            QPushButton:pressed { padding: 5px; }
        """)
        m_layout.addWidget(self.btn_map_close)
    ...
    def center_popup(self, box):
        qr = box.frameGeometry()
        cp = self.main_window.rect().center()
        qr.moveCenter(cp); box.move(qr.topLeft())
