...
class RobotControlPanel(QWidget):
    ...
    # DB 및 알림 관련 시그널 (필요 시 로직 핸들러에서 연결 가능)
    ...
    def _setup_right_panel(self):
        ...
        # 페이지 구성 추가
        self._setup_remote_page() # [PAGE 1]
        self._setup_db_page()     # [PAGE 2]
        self._setup_alarm_page()  # [PAGE 3] 신규 추가

        self.right_stack.addWidget(self.page_main)   # Index 0
        self.right_stack.addWidget(self.page_remote) # Index 1
        self.right_stack.addWidget(self.page_db)     # Index 2
        self.right_stack.addWidget(self.page_alarm)  # Index 3 신규 추가
    ...
    def _setup_db_page(self):
        self.page_db = QWidget()
        layout = QVBoxLayout(self.page_db); layout.setContentsMargins(0, 0, 0, 0)
        db_frame = QFrame()
        db_frame.setStyleSheet("QFrame { background-color: white; border-radius: 20px; border: 1px solid #CFD8E3; }")
        inner = QVBoxLayout(db_frame); inner.setContentsMargins(20, 25, 20, 25)

        title = QLabel("🗄 데이터베이스 조회"); title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet("font-size: 22px; font-weight: 900; color: #2C3E50; margin-bottom: 10px;")
        ...
        self.db_table.setStyleSheet("""
            QTableWidget { border: none; background-color: white; alternate-background-color: #F8F9FA; font-size: 13px; }
            QHeaderView::section { background-color: #AED9FF; padding: 10px; font-weight: bold; border: 1px solid #DDE4ED; }
            QTableWidget::item { padding: 5px; border-bottom: 1px solid #F0F4F8; }
        """)
        ...
        inner.addWidget(self.db_table)

        self.btn_db_back = QPushButton("◀ 메인 화면으로 돌아가기")
        self.btn_db_back.setFixedHeight(60)
        self.btn_db_back.setStyleSheet("QPushButton { background-color: #333; color: white; border-radius: 15px; font-weight: bold; } QPushButton:hover { background-color: #555; }")
        inner.addWidget(self.btn_db_back)
        layout.addWidget(db_frame)

    def _setup_alarm_page(self):
        """[PAGE 3] 재고 부족 알림 화면 구성"""
        self.page_alarm = QWidget()
        layout = QVBoxLayout(self.page_alarm); layout.setContentsMargins(0, 0, 0, 0)
        alarm_frame = QFrame()
        alarm_frame.setStyleSheet("QFrame { background-color: white; border-radius: 20px; border: 1px solid #CFD8E3; }")
        inner = QVBoxLayout(alarm_frame); inner.setContentsMargins(20, 25, 20, 25)

        title = QLabel("🚨 실시간 재고 부족 알림"); title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet("font-size: 22px; font-weight: 900; color: #E86464; margin-bottom: 10px;")
        inner.addWidget(title)

        self.alarm_table = QTableWidget()
        self.alarm_table.setColumnCount(4)
        self.alarm_table.setHorizontalHeaderLabels(["우선순위", "제품명", "현재수량", "상태"])
        self.alarm_table.setStyleSheet("""
            QTableWidget { border: none; background-color: white; alternate-background-color: #FFF5F5; font-size: 14px; }
            QHeaderView::section { background-color: #FFDADA; padding: 10px; font-weight: bold; border: 1px solid #DDE4ED; color: #C0392B; }
            QTableWidget::item { padding: 8px; border-bottom: 1px solid #F0F4F8; }
        """)
        ...
        inner.addWidget(self.alarm_table)

        self.btn_alarm_back = QPushButton("◀ 메인 화면으로 돌아가기")
        self.btn_alarm_back.setFixedHeight(60)
        self.btn_alarm_back.setStyleSheet("QPushButton { background-color: #333; color: white; border-radius: 15px; font-weight: bold; } QPushButton:hover { background-color: #555; }")
        inner.addWidget(self.btn_alarm_back)
    ...
    def _setup_popups(self):
        ...
        self.btn_yes.setStyleSheet("QPushButton { background-color: #28A745; color: white; border-radius: 18px; font-size: 20px; font-weight: bold; } QPushButton:hover { background-color: #218838; }")
        self.btn_no = QPushButton("아니오 ⚓"); self.btn_no.setFixedHeight(65)
        self.btn_no.setStyleSheet("QPushButton { background-color: #E86464; color: white; border-radius: 18px; font-size: 20px; font-weight: bold; } QPushButton:hover { background-color: #C0392B; }")
        ...
        self.btn_map_close.setStyleSheet("QPushButton { background-color: #28A745; color: white; border-radius: 18px; font-size: 22px; font-weight: bold; } QPushButton:hover { background-color: #218838; }")
    ...
    def _connect_internal_events(self):
        ...
        self.btn_db.clicked.connect(self.switch_to_db_page)
        self.btn_alarm.clicked.connect(self.switch_to_alarm_page) # 신규 추가

        self.btn_back.clicked.connect(lambda: self.right_stack.setCurrentIndex(0))
        self.btn_db_back.clicked.connect(lambda: self.right_stack.setCurrentIndex(0))
        self.btn_alarm_back.clicked.connect(lambda: self.right_stack.setCurrentIndex(0)) # 신규 추가
    ...
    def switch_to_db_page(self):
        self.load_dummy_db_data()
        self.right_stack.setCurrentIndex(2)
        self.dbRefreshRequested.emit()

    def switch_to_alarm_page(self):
        self.load_dummy_alarm_data()
        self.right_stack.setCurrentIndex(3)
        self.alarmRefreshRequested.emit()

    def load_dummy_db_data(self):
        sample_data = [("식료품", "냉동 피자", "QR_001", "15", "11:20:05", "A-1"), ("음료", "콜라 500ml", "QR_002", "30", "11:21:10", "B-3")]
        self.db_table.setRowCount(len(sample_data))
        for r, row_data in enumerate(sample_data):
            for c, value in enumerate(row_data): self.db_table.setItem(r, c, QTableWidgetItem(value))

    def load_dummy_alarm_data(self):
        alarm_data = [("🔴 긴급", "삼다수 2L", "2개", "채움 필요"), ("🟠 주의", "육개장 사발면", "5개", "발주 권장")]
        self.alarm_table.setRowCount(len(alarm_data))
        for r, row_data in enumerate(alarm_data):
