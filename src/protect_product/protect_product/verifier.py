...
class VerifierNode(Node):
    def __init__(self):
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer([self.img_sub, self.det_sub], 10, 0.1)
    ...
    def sync_callback(self, img_msg, det_msg):
        ...
        cv2.imshow("PC_Verifier", frame); cv2.waitKey(1)

def main():
    rclpy.init(); node = VerifierNode(); rclpy.spin(node)
