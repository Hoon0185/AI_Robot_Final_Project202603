...
{
  // ROS 메시지 형태를 계산하기 편한 2D 좌표로 변환
  geometry_msgs::msg::Pose2D current_pose2d;
  current_pose2d = nav_2d_utils::poseToPose2D(current_pose.pose);

  // 1. 로봇이 멈춰 있다면 (변위가 0.01m 미만일 때)
  if (std::abs(current_pose2d.x - baseline_pose_.x) < 0.01 &&
      std::abs(current_pose2d.y - baseline_pose_.y) < 0.01)
  {
    // 마지막 이동 시각을 계속 '지금'으로 갱신하여 10초 타임아웃을 방지합니다.
    baseline_time_ = clock_->now();
    return true;
  }

  // 2. 기존 Nav2 로직 유지 (로봇이 움직이기 시작하면 다시 정상 감시)
