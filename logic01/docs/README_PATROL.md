...
- **연동 파라미터**: `avoidance_wait_time` (초). 서버 DB의 설정값과 실시간으로 동기화됩니다.
...
- **우회 로직 최적화**: 좁은 공간에서의 효율을 위해, 장애물 감지 시 짧은 회전(0.5초) 후 즉시 주행 재시도를 수행하도록 최적화되었습니다.
...
- **초기 위치 보정 (Improved Covariance)**: `initialpose` 발행 시 공분산 값을 정밀하게 설정하여 AMCL의 위치 추정 신뢰도를 높였습니다.
- **네임스페이스 지원**: 모든 서비스 경로를 절대 경로(예: `/obstacle_node/set_parameters`)로 사용하여 확장성을 높였습니다.
- **주행 연속성 보장**: 이전에는 주행 중 취소(`CANCELED`)나 중단(`ABORTED`) 발생 시 순찰이 종료되었으나, 현재는 상태를 유지하며 자동으로 다음 행동(재시도 또는 다음 지점 이동)을 결정합니다.
