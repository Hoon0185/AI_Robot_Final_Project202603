...
- **주행 안정성 (LOGIC_02 통합)**: 
    - **지능형 속도 제어**: 장애물 감지 시 Nav2의 최대 선속도(`FollowPath.max_vel_x`)를 0.0으로 동적으로 변경하여 즉시 정지합니다.
    - **가상 벽(Virtual Wall) 활용**: `/scan_virtual` 토픽을 통해 전방에 가짜 장애물을 생성함으로써, Nav2가 스스로 안전한 우회 경로를 계산하도록 유도합니다.
    - **주행 복구(Auto Retry)**: `ABORTED` 발생 시 2초 후 자동 재시도로 주행 연속성을 극대화했습니다.
...
## 6. 특징 및 권장 설정
- **지능형 가단 대응 (Virtual Obstacle)**: `obstacle_node`가 발행하는 `/scan_virtual`은 Nav2 전용 장애물 레이어로 활용됩니다.
