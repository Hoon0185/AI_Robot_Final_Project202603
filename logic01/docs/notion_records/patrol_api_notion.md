...
2. **상품 판독 메커니즘 변경**: 
    ...
    * 해결: **바코드(Barcode)** 기반 판독 체계로 전환하여 서버에서 직접 데이터 정합성을 검증하도록 고도화. (v0.0.18+: `yolo_class_id` 및 `status` 필드 추가)
3. **API 접속 주소 및 세션 관리 최적화 (v0.0.18+)**:
    * 문제: `/api` 접두사 사용으로 인한 404 에러 및 순찰 종료 리포팅 누락 현상.
    * 해결: 모든 엔드포인트에서 `/api` 경로를 제거하고 8000번 포트 루트로 통일. 순찰 종료 시 API와 DB 직접 업데이트를 병행하는 이중 보강 로직 적용.
4. **배터리 수치 보고 정밀화 (v0.0.18+)**:
    * 문제: 로봇의 배터리 잔량이 0~1 로 보고될 때와 0~100으로 보고될 때를 구분하지 못해 대시보드 그래프가 요동침.
    * 해결: 수신된 수치를 분석하여 자동으로 퍼센트(%) 단위로 정규화하는 전처리 로직을 `patrol_node` 및 `InventoryDB` 연동부에 적용.
5. **토픽 이름 충돌 및 절대 경로화**:
