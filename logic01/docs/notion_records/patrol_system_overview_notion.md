...
**37. 배터리 상태 보고 정밀도 및 무결성 개선:**
* 문제: 로봇의 배터리 전압이 0.0~1.0(표준) 또는 0~100(정수)로 혼재되어 보고되어 서버 대시보드 표시가 불안정함.
* 해결: `patrol_node` 내에 정규화 로직을 추가하여 모든 범위를 퍼센트(%) 단위로 자동 변환하고, NaN/Inf 값에 대한 방어 코드를 구현함.

**38. 내비게이션 중단 상태(Status 5/6) 회복력 강화:**
* 문제: 주행 중 장애물 등으로 인해 `ABORTED` 또는 `CANCELED` 발생 시 순찰 시퀀스가 비정상 종료됨.
* 해결: 해당 상태 수신 시 즉시 종료하는 대신 3초간 안정화 대기 후 재전송하거나 상태를 유지하는 로직을 추가하여 순찰 지속성을 확보함.

**39. 비전 송출 안정화 (RTSP over TCP & FPS Limiting):**
* 문제: WiFi 환경에서 UDP 기반 RTSP 스트림의 패킷 손실로 인해 화면 깨짐 및 지연이 누적됨.
* 해결: `rtsp_transport;tcp` 옵션을 강제하고, 송출 프레임을 5 FPS로 제한하여 대역폭 사용량을 75% 절감함으로써 실시간 안정성을 극대화함.

**40. 순찰 시작 전 명령 큐 강제 정화 (Zombie Goal Cleanup):**
* 문제: 이전 주행 명령이 남아있는 상태에서 새로운 순찰이 시작될 때 로봇이 엉뚱한 방향으로 튀는 현상 발생.
* 해결: `START_PATROL` 명령 수신 즉시 현재 진행 중인 모든 Nav2 목표를 비동기적으로 취소(`cancel_goal_async`)하는 전처리 로직을 추가함.

**41. API 서버 접속 주소 표준화:**
* 문제: 노드별로 `/api` 접두사 사용 여부가 달라 통신 연결 실패 사례 빈번.
* 해결: `InventoryDB` 및 모든 모듈에서 `/api` 경로를 삭제하고 8000번 포트의 루트 경로를 사용하도록 단일화함.

**42. 초기 위치 유령 도착(Ghost Arrival) 방지:**
* 문제: 노드 시작 시 (0,0,0)으로 강제 발행하던 `initialpose` 타이머가 실제 위치와 충돌하여 내비게이션 오작동 유발.
* 해결: 자동 초기화 로직을 제거하고, 사용자가 UI 또는 RViz를 통해 명시적으로 위치를 잡도록 가이드를 수정하여 주행 신뢰성을 보장함.
</aside>
