...
```
> [!IMPORTANT]
> 로봇 네임스페이스(예: `TB3_2`)를 사용 중인 경우, 모든 명령어 뒤에 `__ns:=/TB3_2` 옵션을 추가해야 합니다. 또한, 수동/분할 실행 시 `cmd_vel:=cmd_vel_nav` 리매핑을 반드시 포함해야 장애물 회피 멀티플렉서가 작동합니다.
```
> [!TIP]
...
### A. 통합 노드 활성화 (패키지 포함)
시스템 실행 시 `run_rfid:=true` 인자를 추가하면 `rfid_localization_node`가 함께 자동 실행됩니다.
```bash
...
### B. 독립형 노드 실행 (Standalone)
패키지 전체를 빌드하거나 소싱하기 번거로운 하드웨어(라즈베리파이) 환경에서 단독으로 실행하여 위치를 보정할 수 있습니다.
```bash
# src 폴더 내부에서 실행
...
3. 생성된 메시지를 `/initialpose` 토픽으로 발행하여 AMCL의 현재 위치를 높은 신뢰도로 강제 업데이트합니다.

### D. 태그 및 좌표 관리
`logic01/src/patrol_main/patrol_main/rfid_localization_node.py` (통합형) 또는 `src/rfid_robot_node.py` (독립형) 파일의 `self.landmark_map` 딕셔너리에 태그 ID와 실제 지도상의 좌표를 입력하여 확장할 수 있습니다.
