...
class ObstacleNode(Node):
  def __init__(self):
    ...
    # 시간 및 카운터 변수
    self.clear_count = 0 # 장애물 소멸 지속 시간 체크용 카운터
    self.wait_counter = 0 # 대기시간 측정
    # 설정 및 데이터 저장 변수
    self.safe_distance = 0.50 # 정지 기준 안전거리
      ...
      if min_distance < check_dist:
        ...
        if self.is_blocked and self.wait_counter >= 0:
          ...
          if self.clear_count > self.timer_second * 0.5: # 0.5초 이상 장애물 사라짐 -> 재주행
            ...
            msg_go = Bool()
            msg_go.data = False
            self.emergency_pub.publish(msg_go)
  ...
  def timer_callback(self):
    ...
    if self.is_blocked:
      if not self.is_teleop_active:
        self.stop_robot() # 장애물 감지 후 수동 조작이 없으면 계속 정지 유지
      self.wait_counter += 1
